...
public class MenuCandidat extends BeanAdapter {
	...
	public String Suivant() {
		...
		FacesUtil.setApplicationMapValue("theCandidat", getCandidatSelected());
		
		Utils.Log( "Le candidat en session : "
				+ getCandidatSelected()   );
	...
	public List<Candidat> getMesElements() {
		...
		for (Candidat cand : getMaListe()) {
			...
			Candidat c = new Candidat(cand.getCandidatid(),
					cand.getCandidatCivilite(), cand.getCandidatNom(),
					cand.getCandidatPrenom(), cand.getCandidatMail(),
					cand.getCandidatDateNaissance());
			mesElements.add(c);
		}
		return mesElements;
	}
	...
	private List<Candidat> getMaListe() {
		return PROVIDER.CANDIDATDAO.findAll();
	}

	/**
	 * @param mesElements
	 *            the mesElements to set
	 */
	...
	/**
	 * @param candidatSelected
	 *            the candidatSelected to set
	 */
	...
	private void afficheCandidat() {
		Candidat theCandidat = PROVIDER.CANDIDATDAO.find(Long.parseLong(getMaValeur()));
		List list = new ArrayList<Candidat>();
		list.add(theCandidat);
		setList((ArrayList<Candidat>) list);
		this.setCandidatSelected(theCandidat);

		setTheDate(Utils.dateToMySQLString(theCandidat
				.getCandidatDateNaissance()));
