...
public class TheCodeTester implements Serializable {
	...
	// ok
	private String urlPHP ="http://localhost:8080/CodeTester/faces/PHP/code.php";
	...
	public TheCodeTester() {
		...
		//"C:\\Project\\Workspace\\CodeTester\\target\\CodeTester-1.0-SNAPSHOT\\PHP\\";
		
		
		// myPath2 = "C:\\Project\\Workspace\\CodeTester\\src\\main\\webapp\\PHP\\";
		
		myPath2 ="C:\\Project\\Workspace\\CodeTester\\target\\CodeTester-1.0-SNAPSHOT\\PHP\\"; 
		pathCodePHP = myPath2 + "code.php";

		
		Passage thePassage = (Passage) FacesUtil
				.getApplicationMapValue("passageEnCours");
		Utils.Log("thePassage : "+thePassage);
		

		// récupérer le test en session - id du test / liste de questions du
		// test
		int passage_id = thePassage.getPassageid();
		theTest = (Test) FacesUtil.getApplicationMapValue("testEnCours");
		Utils.Log("theTest : "+theTest);
		


		
		String test_id = theTest.getTestid().toString();

		System.out.println("passage_id=" + passage_id + " test_id=" + test_id);

		// on recupere le tableau en session + rep des réponses
		this.tab_test = (Hashtable<String, Hashtable<Integer, List<Question>>>) FacesUtil
				.getApplicationMapValue("tab_test");

		
		this.dirNamePHP = (String) FacesUtil
				.getApplicationMapValue("dirNamePHP");
		
		this.dirNameXML = (String) FacesUtil
				.getApplicationMapValue("dirNameXML");
		...
		Hashtable<Integer, List<Question>> tab_rub = (Hashtable<Integer, List<Question>>) tab_test
				.get(testid);
	...
	public String Valider() throws IOException {
		// on récupère le code saisit par le candidat + on l'écrit ds le rep des
		// réponses
		...
		try {
			...
			ex.printStackTrace();
			Logger.getLogger(TestControler.class.getName()).log(Level.SEVERE,
					null, ex);
		}

		// System.out.println("création du xml");
		try {
			Utils.buildXML(getDirNameXML() + "question_" + (getCount())
					+ ".xml", repPHP);
		...
		System.out.println("********************************************");
		System.out.println("count : " + getCount() + " == nb_quest_total : "
				+ getNb_quest_total());
		System.out.println("********************************************");
	...
	private void initQuestion() {
		...
		// on défini le path de l'énoncé de la question en cours //en dur
		pathEnoncePHP = "C:\\Project\\Workspace\\CodeTester\\ENONCES\\question_"
				+ id_quest + ".php";
		
		// on défini les path des images associées à la question en cours
		setImage("../resources/images/enonces_PHP/boucles/large/boucle_"
				+ id_quest + ".JPG");
		
		setImage_small("enonces_PHP/boucles/small/boucle_" + id_quest + ".JPG");
		
		setImage_resultat("../resources/images/enonces_PHP/boucles/resultat/boucle_"
				+ id_quest + ".JPG");
		
		// on vérifie que ces images existent
		File f = new File(
				"C:\\Project\\Workspace\\CodeTester\\target\\CodeTester-1.0-SNAPSHOT\\resources\\images\\enonces_PHP\\boucles\\large\\boucle_"
						+ id_quest + ".JPG");
		File f2 = new File(
				"C:\\Project\\Workspace\\CodeTester\\target\\CodeTester-1.0-SNAPSHOT\\resources\\images\\enonces_PHP\\boucles\\resultat\\boucle_"
						+ id_quest + ".JPG");
		...
		// System.out.println("Image_resultat_exist=" +
		// getImage_resultat_exist());
		// System.out.println("Image_exist=" + getImage_exist());
		// on défini l'énoncé
		enonce = getThe_list().get(getCount()).getQuestiontext();
		setEnonce(enonce);
		// on définie sa rubrique
		setRubrique(getThe_list().get(getCount()).getRubriqueid()
				.getRubriqueid().toString());
	}
	...
	private void dureeToString(int dureeTest) {
		...
		setTime(Utils.twoDigitString(minutes) + " : "
				+ Utils.twoDigitString(seconds));
	}
	...
	/**
	 * @param pathApplication
	 *            the pathApplication to set
	 */
	...
	/**
	 * @param codeIsCompiled
	 *            the codeIsCompiled to set
	 */
	...
	/**
	 * @param aNb_quest_total
	 *            the nb_quest_total to set
	 */
	...
	/**
	 * @param image_resultat
	 *            the image_resultat to set
	 */
	...
	/**
	 * @param image_resultat_exist
	 *            the image_resultat_exist to set
	 */
